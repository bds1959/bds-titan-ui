...
useEffect(() => { 
  // if (isLoaded === false) {
    axios.get(`http://172.16.1.240:32454/api/v1/encs-dc-asset-allocation`,
    {
      headers: {               
...
//-----------------------------------------Delete method----------------------------------------------------------------------
 const removeData = (iscTechCategoryName) => {
  ...
  axios.delete(`http://172.16.1.240:32454/api/v1/encs-dc-asset-allocation/${iscTechCategoryName}`,
  {
    headers: {               
      "Content-Type": "application/json",
      "access-token": `${token}`
 ...
 const columns = [
  ...
  },
  {
    ...
    cell: row => (
      <div className='d-flex align-items-center'>
        <div className='user-info text-truncate '>
          <span className='d-block font-weight-bold text-truncate'>{row.dcAssetId}</span>
          {/* <small>{row.post}</small> */}
        </div>
      </div>
    )
