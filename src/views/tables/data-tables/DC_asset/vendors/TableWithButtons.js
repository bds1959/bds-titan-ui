...
useEffect(() => { 
  // if (isLoaded === false) {
    axios.get(`http://172.16.1.240:32454/api/v1/encs-dc-asset-vendors`,
    {
      headers: {               
...
//-----------------------------------------Delete method----------------------------------------------------------------------
 const removeData = (iscTechCategoryName) => {
  ...
  axios.delete(`http://172.16.1.240:32454/api/v1/encs-dc-asset-vendors/${iscTechCategoryName}`,
  {
    headers: {               
      "Content-Type": "application/json",
      "access-token": `${token}`
 ...
 const columns = [
  ...
  },
  {
    ...
    cell: row => (
      <div className='d-flex align-items-center'>
        <div className='user-info text-truncate '>
          <span className='d-block font-weight-bold text-truncate'>{row.dcAssetVendorDesc}</span>
          {/* <small>{row.post}</small> */}
        </div>
      </div>
    )
    ...
    <Fragment>
      <Card>
        <CardHeader className="flex-md-row flex-column align-md-items-center align-items-start border-bottom">
          <CardTitle tag="h4" style={{ color: 'white'}}>DC ASSET VENDOR</CardTitle>
